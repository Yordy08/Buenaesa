<template>
    <div class="container mt-5 pt-5">
      <h1 class="mb-4 text-center" style="color: #0FA6A6;">Noticias de Personajes</h1>
        <hr>
      <!-- Contenedor de tarjetas -->
      <div class="d-flex flex-wrap justify-content-center">
        <div 
          v-for="noticia in noticiasPersonajes" 
          :key="noticia._id" 
...
<div class="mt-auto text-center">
      ...
      <div v-if="noticiasPersonajes.length === 0" class="text-center mt-5">
        <h4>No hay noticias de Personajes aún.</h4>
      </div>
    </div>
  </template>
  ...
  const obtenerNoticias = async () => {
    try {
      ...
      noticiasPersonajes.value = noticias.value.filter(
        noticia => noticia.categoria === 'Personajes'
      )
  
      console.log('Noticias de Personajes:', noticiasPersonajes.value)
